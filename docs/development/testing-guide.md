...
本指南详细说明了 Shield 项目的测试策略、工具使用和最佳实践，确保代码质量和系统可靠性。基于最新的服务层单元测试架构，提供全面的测试开发指导。
...
shield/
├── test/                           # 集成测试和专门单元测试
│   ├── test_helpers.go            # 测试辅助函数和标准测试用户
│   ├── permission_system_test.go  # 权限系统集成测试
│   ├── permission_filtering_test.go # 权限过滤单元测试
│   ├── user_service_test.go       # 用户服务单元测试 ✨
│   ├── permission_service_test.go # 权限服务单元测试 ✨
│   ├── role_service_test.go       # 角色服务单元测试 ✨
│   ├── blacklist_service_test.go  # 黑名单服务单元测试 ✨
│   ├── redis_test.go              # Redis 集成测试
│   ├── logger_test.go             # 日志测试
│   └── validator_test.go          # 验证器测试
├── internal/
│   ├── handlers/                  # Handler 层测试（待补充）
│   │   └── *_test.go
│   ├── services/                  # Service 层内部测试（如需要）
│   │   └── *_test.go
│   └── repositories/              # Repository 层测试（如需要）
│       └── *_test.go
└── pkg/                           # 公共包测试
    └── *_test.go
```

**注意**: ✨ 标记的是新增的核心服务单元测试，提供完整的业务逻辑测试覆盖。

## 🧪 单元测试实践

### 标准测试用户系统

Shield 项目使用标准测试用户系统，避免在每个测试中重复创建用户数据：

```go
// 获取标准测试用户
testUsers := SetupStandardTestUsers(db)
systemAdmin := testUsers["admin@system.test"]  // 系统管理员
tenantAdmin := testUsers["admin@tenant.test"]  // 租户管理员
regularUser := testUsers["user@tenant.test"]   // 普通用户
...
基于新的服务层单元测试架构，更新的覆盖率要求：

| 层级 | 最低覆盖率 | 目标覆盖率 | 当前状态 |
|------|-----------|-----------|----------|
| 核心服务层 | 90% | 95% | ✅ 已达标 |
| Handler | 60% | 80% | 🔄 待补充 |
| Repository | 70% | 85% | ✅ 通过集成测试覆盖 |
| 权限系统 | 95% | 100% | ✅ 已达标 |
| 认证授权 | 90% | 95% | ✅ 已达标 |
| 整体项目 | 75% | 85% | ✅ 已达标 |

**核心服务层测试覆盖**：
- ✅ **用户服务**: 创建、认证、验证、CRUD 全覆盖
- ✅ **权限服务**: 检查、列表、树形结构、菜单权限 全覆盖
- ✅ **角色服务**: CRUD、权限分配、查询 全覆盖
- ✅ **黑名单服务**: 查询、统计、同步、批量操作 全覆盖
...
- 📋 [测试速查手册](./testing-cheatsheet.md) - 常用测试命令和模式快速参考 ⚡
- 🔧 [服务层测试指南](./service-testing-guide.md) - 服务层单元测试详细指南
- 👥 [测试用户指南](./test-users.md) - 标准测试用户系统使用
